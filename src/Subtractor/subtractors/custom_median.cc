...
Subtractors::Median::Median(QJsonObject const &a_config)
    : m_history{a_config[HISTORY].toInt()},
      m_historyDelta{a_config[HISTORYDELTA].toInt()}, m_iteration(0),
      m_firstInitialization(true),
      m_firstTime(true), m_decisionThreshold{
                             a_config[DECISION_THRESHOLD].toInt()} {
  m_backgroundSubtractor =
      Subtractors::imgMedian(static_cast<unsigned char>(m_history));
}
...
cv::Mat Subtractors::Median::getBackground() { return m_background; }
